...
function sendAccountCreatedSMS(fullName, userAccount, phoneNumber) {
  const message =
    `Hi ${fullName}, your ${userAccount.accountTypeName} Account has ` +
...
function sendDebitTransactionEmail(user, transaction, message) {
  const transactionDate = new Date();

  if (!message) {
    message =
      `Dear ${user.firstName} ${user.lastName}, ` +
      `Your account has been debited with amount of USD ` +
      `${transaction.amount.toFixed(2)} on ${transactionDate.toDateString()} ` +
      ``;
  }
  ...
  return sendMail({
    recipient: user.emailAddress,
    subject: "Lollands Credit Union Account Debit Transaction",
    html: message,
    text: message,
    altText: message,
  });
...
function sendWelcomeMail(fullName, recipientAddress) {
  const mailText =
    `Welcome to Lollands Credit Union ${fullName}. <br />` +
